...
from inventories.models import L3vpn

def add_service(name, identity, state, comments, created, updated, client_id, service_type):
    s = L3vpn.objects.get_or_create(
        name = name,
        id_csd = identity,
        state = state,
        comments = comments,
...
def populate():
    ...
    for element in root.findall('.//{services.schema.networkapi.jmp.juniper.net}Service'):
        ...
        instance_type = element.find('.//{services.schema.networkapi.jmp.juniper.net}InstanceType').text
        ...
        service_definition_name = element.find('.//{services.schema.networkapi.jmp.juniper.net}ServiceDefinitionName').text
        ...
        a = add_service(name, identity, state, comments, create_date, last_update_date, client_id, service_type)
        print(a)
